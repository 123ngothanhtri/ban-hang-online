...
class SanphamController extends Controller
{
    public function index()
    {
        ...
        // ->select('')
        // ->selectRaw('(select SUM(quantity) FROM chitietdonhang WHERE chitietdonhang.id_sanpham=sanpham.id_sanpham) as sum ')
        // ->havingRaw('sum is not null')
        // ->orderByRaw('sum desc')
        // ->get();
        // ->join('loaisanpham', 'sanpham.id_loaisanpham', 'loaisanpham.id_loaisanpham')->get();

        $data = DB::table('sanpham')
        ->join('loaisanpham', 'sanpham.id_loaisanpham', 'loaisanpham.id_loaisanpham')
        ->get();
        return view('admin.SanPham',compact('data'));
    }
